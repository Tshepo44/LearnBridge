/* counsellor-portal.js
   Counsellor Portal for LearnBridge (University of Johannesburg)
   ...
   - Notifications center (in-memory + stored under counsellorData[counsellorId].notifications)
   - Sessions history and export
   - Uses same unified storage structure and updates counsellingRequests entries
   - Starts empty (0 requests, 0 counsellors) if storage empty
*/
...
(() => {
  ...
  function loadCounselling(){ const db = loadDB(); db.counsellingRequests = db.counsellingRequests || []; return db.counsellingRequests; }{ const db = loadDB(); db.counsellingRequests = db.counsellingRequests || []; return db.counsellingRequests; }
  function saveCounselling(list){ const db = loadDB(); db.counsellingRequests = list; saveDB(db); }

  function loadCounsellorData(){ const db = loadDB(); db.counsellorData = db.counsellorData || {}; return db.counsellorData; }{ const db = loadDB(); db.counsellorData = db.counsellorData || {}; return db.counsellorData; }
  function saveCounsellorData(obj){ const db = loadDB(); db.counsellorData = Object.assign(db.counsellorData || {}, obj); saveDB(db); }

  function recordAudit(action, by='counsellor', details=''){
    const db = loadDB(); db.audit = db.audit || []; db.audit.unshift({ id: uid(), action, by, details, time: now() }); saveDB(db);
  }

  /* ---------- Ensure DB exists ---------- */
  if(!localStorage.getItem(STORAGE_KEY)){
    saveDB({ users: [], audit: [], sessions: {}, counsellorData:{}, studentData:{}, counsellorData:{}, counsellingRequests:[], counsellingRequests:[] });
  }

  /* ---------- Simple auth simulation (pick a counsellor) ---------- */
  // For demo purposes the counsellor portal allows selecting one of the existing counsellor users to 'sign in'.
  function getFirstCounsellor(){ return loadUsers().find(u=>u.role==='counsellor') || null; }{ return loadUsers().find(u=>u.role==='counsellor') || null; }
    ...
    document.body.innerHTML = `
      <div style="width:100%;max-width:1200px;display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        <div style="display:flex;align-items:center;gap:12px">
          ...
          <div style="color:#fff;font-weight:700;font-size:1.2rem">University of Johannesburg — Counsellor Portal</div>
        </div>
      </div>
      ...
      <div class="portal" id="portalRoot">
        <aside class="side">
          ...
          <div style="padding:12px;color:rgba(255,255,255,.85)">
            <div style="margin-bottom:8px">Counsellor • UJ</div>
            <div style="font-size:12px">Signed in as: <span id="counsellorNameMuted">—</span></div>
        ...
        <main class="content" id="mainContent">
          <div class="topbar">
            <div><strong>Counsellor Portal</strong> <span class="muted">— Manage booking requests & availability</span></div>
            <div class="flex">
  ...
  function render(){
    ...
    // auto sign-in demo: pick first counsellor if exists
    const t = getFirstCounsellor();
    if(t){ signInAsCounsellor(t.id); }
    showView('dashboard');
  }
  ...
  /* ---------- Counsellor context ---------- */
  let CURRENT_COUNSELLOR_ID = null;
  function signInAsCounsellor(counsellorId){ CURRENT_COUNSELLOR_ID = counsellorId; const users = loadUsers(); const u = users.find(x=>x.id===counsellorId) || {}; document.getElementById('counsellorNameMuted').textContent = u.name || '—'; ensureCounsellorProfileExists(counsellorId); }
  function signOut(){ CURRENT_COUNSELLOR_ID = null; document.getElementById('counsellorNameMuted').textContent = '—'; }

  document.addEventListener('click', (e)=>{ if(e.target && e.target.id==='signOutBtn'){ signOut(); alert('Signed out (demo)'); } });

  function ensureCounsellorProfileExists(id){ const db = loadDB(); db.counsellorData = db.counsellorData || {}; if(!db.counsellorData[id]){ db.counsellorData[id] = { profile:{}, availability:[], notifications:[], stats:{}, settings:{} }; saveDB(db); } }

  /* ---------- DASHBOARD ---------- */
  function renderDashboard(){
    const requests = loadCounselling();
    const myReq = requests.filter(r=>r.personId===CURRENT_COUNSELLOR_ID);
    const counts = {
      ...
      <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
          <div>
            ...
            <div class="small-muted">Summary of your counselloring activity</div>
          </div>
  ...
  function renderRequestsView(){
    ...
    const mine = all.filter(r=>r.personId===CURRENT_COUNSELLOR_ID).sort((a,b)=> new Date(b.createdAt)-new Date(a.createdAt));
    ...
    function handleAction(action, id){
      const all = loadCounselling(); const idx = all.findIndex(a=>a.id===id); if(idx===-1) return alert('Request not found'); const item = all[idx];
      if(action==='accept'){
        all[idx].status = 'accepted'; all[idx].updated = true; saveCounselling(all); recordAudit('Counsellor accepted request', CURRENT_COUNSELLOR_ID, id); drawReqTab(getActiveTab()); notifyStudent(item.studentId, `Your counselloring request (${item.module||'—'}) was accepted by counsellor.`); alert('Accepted');
      }
      else if(action==='decline'){
        const reason = prompt('Optional reason for decline (will be sent to student)') || 'Declined by counsellor'; all[idx].status='cancelled'; all[idx].comment = (all[idx].comment || '') + '\nCounsellor Decline: ' + reason; saveCounselling(all); recordAudit('Counsellor declined request', CURRENT_COUNSELLOR_ID, id); drawReqTab(getActiveTab()); notifyStudent(item.studentId, `Your counselloring request was declined: ${reason}`); alert('Declined'); }
      else if(action==='complete'){
        all[idx].status='completed'; all[idx].completedAt=now(); saveCounselling(all); recordAudit('Counsellor marked completed', CURRENT_COUNSELLOR_ID, id); drawReqTab(getActiveTab()); notifyStudent(item.studentId, `Your counselloring session has been marked completed.`); alert('Marked completed'); }
      else if(action==='no-show'){
        all[idx].status='no-show'; saveCounselling(all); recordAudit('Counsellor marked no-show', CURRENT_COUNSELLOR_ID, id); drawReqTab(getActiveTab()); notifyStudent(item.studentId, `Session marked as no-show by counsellor.`); alert('Marked no-show'); }
      else if(action==='reschedule'){
        const dt = prompt('Enter new date-time (YYYY-MM-DD HH:MM)') || ''; if(!dt) return; const iso = (new Date(dt)).toISOString(); all[idx].datetime = iso; all[idx].status='accepted'; saveCounselling(all); recordAudit('Counsellor rescheduled', CURRENT_COUNSELLOR_ID, id); drawReqTab(getActiveTab()); notifyStudent(item.studentId, `Your session was rescheduled to ${formatDate(iso)} by the counsellor.`); alert('Rescheduled'); }
      else if(action==='view'){
  ...
  function renderAvailabilityView(){
    const counsellorData = loadCounsellorData(); const td = counsellorData[CURRENT_COUNSELLOR_ID] || { availability:[], profile:{}};
    ...
    function saveAvail(){ const db = loadDB(); db.counsellorData = db.counsellorData || {}; db.counsellorData[CURRENT_COUNSELLOR_ID] = db.counsellorData[CURRENT_COUNSELLOR_ID] || {}; db.counsellorData[CURRENT_COUNSELLOR_ID].availability = avail; saveDB(db); recordAudit('Updated availability', CURRENT_COUNSELLOR_ID); }
  ...
  function renderProfileView(){
    const users = loadUsers(); const me = users.find(u=>u.id===CURRENT_COUNSELLOR_ID) || {}; const td = loadCounsellorData(); const pdata = (td[CURRENT_COUNSELLOR_ID] && td[CURRENT_COUNSELLOR_ID].profile) || {};

    $('#viewContainer').innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px"><div><div style="font-size:1.2rem;font-weight:700">Profile</div><div class="small-muted">Your public counsellor profile</div></div><button id="backDashProf" class="btn ghost">← Dashboard</button></div>
      <div class="card">
    ...
    $('#p_save').onclick = ()=>{
      const db = loadDB(); db.counsellorData = db.counsellorData || {}; db.counsellorData[CURRENT_COUNSELLOR_ID] = db.counsellorData[CURRENT_COUNSELLOR_ID] || {}; db.counsellorData[CURRENT_COUNSELLOR_ID].profile = { name: $('#p_name').value, department: $('#p_dept').value, modules: $('#p_mods').value, bio: $('#p_bio').value }; 
      // also update users table if exists
      db.users = db.users || []; const idx = db.users.findIndex(u=>u.id===CURRENT_COUNSELLOR_ID); if(idx!==-1){ db.users[idx].name = $('#p_name').value; db.users[idx].department = $('#p_dept').value; db.users[idx].modules = $('#p_mods').value; }
      saveDB(db); recordAudit('Updated counsellor profile', CURRENT_COUNSELLOR_ID); alert('Profile saved'); renderProfileView();
    };
  }

  /* ---------- Notifications View ---------- */
  function renderNotificationsView(){
    const td = loadCounsellorData(); const my = (td[CURRENT_COUNSELLOR_ID] && td[CURRENT_COUNSELLOR_ID].notifications) || [];
    $('#viewContainer').innerHTML = `
  ...
  function markAllRead(){ const db = loadDB(); db.counsellorData = db.counsellorData || {}; if(!db.counsellorData[CURRENT_COUNSELLOR_ID]) return; (db.counsellorData[CURRENT_COUNSELLOR_ID].notifications||[]).forEach(n=>n.read=true); saveDB(db); }

  /* ---------- Sessions History ---------- */
  function renderHistoryView(){
    const all = loadCounselling(); const mine = all.filter(r=>r.personId===CURRENT_COUNSELLOR_ID).sort((a,b)=> new Date(b.createdAt)-new Date(a.createdAt));
    $('#viewContainer').innerHTML = `
  ...
  window.LearnBridgeCounsellorPortal = {
    signInAsCounsellor: (id)=>{ signInAsCounsellor(id); render(); },
    getCurrentCounsellor: ()=> CURRENT_COUNSELLOR_ID
  };
